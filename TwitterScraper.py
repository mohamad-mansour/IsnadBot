...
from sqlalchemy import create_engine, Column, Integer, String, DateTime, inspect, Boolean, desc,true
...
- To upload target user IDs: Use the `/upload-target-accounts/` endpoint, ensuring the provided API key is valid.
...
class IsnadAccount(Base):
    __tablename__ = "isnad_accounts"

    id = Column(Integer, primary_key=True, index=True)
    account_name = Column(String, index=True)
    account_type = Column(String, index=True)
    auth_token = Column(String)
    ct_value = Column(String)
    is_used = Column(Boolean, default=false(),index=True)
    created_at = Column(DateTime, default=datetime.utcnow)
...
# Endpoint to upload Excel file and add data to DB
@app.post("/upload-isnad-accounts/")
async def upload_isnad_accounts(
    api_key: str = Depends(get_api_key),
    file: UploadFile = Depends(is_excel_file),
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            account_data = {
                "account_name": row[column_indices["ACCOUNT_NAME"]],
                "account_type": row[column_indices["ACCOUNT_TYPE"]],
                "auth_token": row[column_indices["AUTH_TOKEN"]],
                "ct_value": row[column_indices["CT_0"]]
            }

            # Check if the record exists based on unique identifier (account_name)
            existing_record = db.query(IsnadAccount).filter(IsnadAccount.account_name == account_data["account_name"]).first()
            ...
            if existing_record:
                ...
                # Create a new record and add it to the database
                db_account = IsnadAccount(**account_data)
                db.add(db_account)

        db.commit()
        logger.info('Request from UserID: ' +
                    api_key+' - Isnad Accounts, Data added or updated in the database successfully')
        return JSONResponse(content={"message": "Isnad Accounts , Data added or updated in the database successfully."}, status_code=200)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error processing Isnad Accounts Excel file: {str(e)}")
    ...
    for target_account in target_accounts:
        if target_account.account_id:
            ...
            try:
                ...
                if is_pin_included == 3:
                    print('In Pin 3 ', len(tweetsScrap[0]['data']['user']['result']['timeline_v2']['timeline']['instructions'][2]['entries']))
                    for entry in tweetsScrap[0]['data']['user']['result']['timeline_v2']['timeline']['instructions'][2]['entries']:
                        if entry['content']['entryType'] in {'TimelineTimelineItem'}:
                            tweet_entry = TweetEntry(entry)
                            if tweet_entry.is_edit_eligible:
                    ...
                    print('In Pin 2 ', len(tweetsScrap[0]['data']['user']['result']['timeline_v2']['timeline']['instructions'][1]['entries']))
                    for entry in tweetsScrap[0]['data']['user']['result']['timeline_v2']['timeline']['instructions'][1]['entries']:
                        if entry['content']['entryType'] in {'TimelineTimelineItem'}:
                            tweet_entry = TweetEntry(entry)
                            if tweet_entry.is_edit_eligible:
                ...
                print(
                    f"An error occurred while processing account_id {target_account.account_id.strip()}: {str(e)}")
...
def send_reply_db(tweet_id):


    Session = sessionmaker(bind=engine)
    session = Session()

    # Query for accounts of type "1" that are not used
    isnad_accounts = (
        session.query(IsnadAccount).filter(IsnadAccount.account_type == "1", IsnadAccount.is_used == false()).all()
    )

    # Check if the query returned no unused accounts
    if not isnad_accounts:
        # Reset is_used for all accounts of type "1" to False and retry the query
        session.query(IsnadAccount).filter(IsnadAccount.account_type == "1").update({IsnadAccount.is_used: false()})
        session.commit()

        isnad_accounts = (
            session.query(IsnadAccount)
            .filter(IsnadAccount.account_type == "1", IsnadAccount.is_used == false())
            .all()
        )

    # Check if there are still no accounts
    if not isnad_accounts:
        print("No available accounts.")
        return

    isnad_account = isnad_accounts[0]
    ...
    try:
        ...
        session.commit()
        account = Account(cookies={"ct0": isnad_account.ct_value.strip(), "auth_token": isnad_account.auth_token.strip()})
        account.reply(tweet_content, tweet_id=tweet_id)
        formatted_url = f"https://twitter.com/itdoesnotmatter/status/{tweet_id}"
        logger.info('User ' +isnad_account.account_name + ' finsihed reply on tweet: ' + str(formatted_url))
        print(f"{isnad_account.account_name} finsihed reply on tweet  {formatted_url}")
        pass
    except Exception as e:
        print(f"Exception in send_tweet Error: {e}")
        send_reply_db(tweet_content)


def initialize_scraper_file() -> Scraper:
    # Read cookies from the file
...
def initialize_scraper() -> Scraper:
    Session = sessionmaker(bind=engine)
    session = Session()
    # Query the IsnadAccount table to get not used accounts with account_type equal to "2"
    not_used_accounts = session.query(IsnadAccount).filter(IsnadAccount.is_used == false(), IsnadAccount.account_type == "2").all()

    if not not_used_accounts:
        # If all accounts are used or none match the criteria, update the is_used flag for all accounts to False
        session.query(IsnadAccount).filter(IsnadAccount.account_type == "2").update({IsnadAccount.is_used: false()})
        session.commit()
    ...
    for isnad_account in not_used_accounts:
        ...
        user = isnad_account.account_name
    ...
    # If no valid scraper is found, you may want to handle this case accordingly
    raise Exception("No valid scraper found in the database with account_type equal to '2'")
